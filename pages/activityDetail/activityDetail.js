        ...
        if (response.statusCode == 200) {
          if (response.data.code == 1) {
            that.setData({
              ...
              mobile: response.data.data.page.mobile
            })
        ...
        if (res.authSetting['scope.userInfo']) {
          wx.getUserInfo({
            ...
            success: function (res) {
              let userInfo = res.userInfo;
              wx.removeStorageSync("baseUrl_to");
              utils.sendRequest(app.globalData.publicAdress + 'addons/cms/wxapp.comment/delComment?token=' + wx.getStorageSync("token"), 'get', { "type": "page","id":id})
                .then(function (response) {
                  if (response.statusCode == 200) {
                    //成功
                    if (response.data.code == 1) {
