...
Page({
  ...
  onLoad: function (options) {
    ...
    utils.sendRequest(app.globalData.publicAdress + 'addons/cms/wxapp.user/mynearuser?token=' + wx.getStorageSync("token"), 'get')
        ...
        if (response.statusCode == 200) {
          if (response.data.code == 1) {
            for (var i = 0; i < response.data.data.length; i++) {
              var parm = [];
              that.data.markers.push({ iconPath: response.data.data[i].avatar, id: i, "latitude": response.data.data[i].lat, "longitude": response.data.data[i].lng, width: 30, height: 30,});
}
            that.setData({
